...
@torch.no_grad()
def prepare_model_inputs(batch, freqs_cis_img):
    if len(batch) > 1:
        (
            latents,
        ...
        latents = latents.to(device, dtype=args.latents_dtype)
        ...
        # Model conditions
        model_kwargs = dict(
            encoder_hidden_states=encoder_hidden_states.to(device, dtype=args.inputs_dtype),
            text_embedding_mask=text_embedding_mask.to(device),
            encoder_hidden_states_t5=encoder_hidden_states_t5.to(device, dtype=args.inputs_dtype),
            text_embedding_mask_t5=text_embedding_mask_t5.to(device),
            image_meta_size=image_meta_size.to(device, dtype=args.inputs_dtype),
            style=style.to(device),
            image_rotary_emb=(
                cos_cis_img.to(device, dtype=args.inputs_dtype),
                sin_cis_img.to(device, dtype=args.inputs_dtype)
            )
        )
    else:
        latents = None
...
def main():
    ...
    pipe = HunyuanDiTPipeline.from_pretrained(
        args.diffusers_pipe_name, cache_dir=args.diffusers_cache_path
    )
    ...
    if args.resume is None or args.resume == '':
        ...
        opt = utils.create_optimizer(args.opt.name, model)
        ...
        opt = utils.create_optimizer(args.opt.name, model, verbose=False)
        opt.load_state_dict(opt_dict['state_dict'])
        scheduler = torch.optim.lr_scheduler.ExponentialLR(opt, gamma=0.9998)
        scheduler.load_state_dict(opt_dict['scheduler'])
    ...
    training_prolog = utils.TrainingLoopProlog()
    torch.xpu.reset_accumulated_memory_stats()
    # Training loop
    for epoch in range(start_epoch, args.epochs):
        training_prolog.step(epoch, training_data, data_encoder)
        ...
        for batch in tqdm(data_loader, miniters=1):
            ...
            latents, model_kwargs = prepare_model_inputs(batch, freqs_cis_img)
        ...
        if (epoch+1) % args.ckpt_every_epoch == 0:
            save_name = f'./{args.output_folder}/{args.LORA_NAME}_{args.training_parts}_{epoch+1:04d}'
            model.save_pretrained(save_name)
            torch.save(
                {
                    'optimizer_class': args.opt.name,
